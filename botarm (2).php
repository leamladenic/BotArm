...
<head>
	...
	<script>
	//MQTT
	const client = new Paho.MQTT.Client("ws://iot.eclipse.org/ws", "botarm-021201" + new Date().getTime());
	const topic = "BotArm";
	...
	const publish = (topic, ang) => { 
		let angels = new Paho.MQTT.Message(ang);
		angels.destinationName = topic;
		client.send(angels);
	};
	...
	if(window.innerWidth < 650){
		...
		var zone_B_radius = 45;
		var zone_B_center_x = 0.65*pivot_x;
		var zone_B_center_y = -1.4*(zone_B_radius + SOLE_HEIGHT + 10);
		var zone_B_last_angle = 0;

		var zone_C_radius = 45;
		var zone_C_center_x = -0.65*pivot_x;
		var zone_C_center_y = -1.4*(zone_B_radius + SOLE_HEIGHT + 10);
		var zone_C_last_angle = 0;

		var zone_D_width = 15;
		var zone_D_height = 100.0;
		var zone_D_center_x = 0;
		var zone_D_center_y = -(zone_B_radius + SOLE_HEIGHT + 50);
		var grip_height = 40;
		var grip_width = 70;

		var canvas_width = zone_A_radius * 2 + 10;
		var canvas_height = zone_A_radius  + SOLE_HEIGHT + zone_B_radius * 2 + 100;
	}
	else {
		var ARM_LENGTH_1 = 95;
		var ARM_LENGTH_2 = 88;
		var ARM_LENGTH_3 = 100;
		...
		var zone_B_radius = 85;
		var zone_B_center_x = 0.65*pivot_x;
		var zone_B_center_y = -1.5*(zone_B_radius + SOLE_HEIGHT + 20);
		var zone_B_last_angle = 0;

		var zone_C_radius = 85;
		var zone_C_center_x = -0.65*pivot_x;
		var zone_C_center_y = -1.5*(zone_B_radius + SOLE_HEIGHT + 20);
		var zone_C_last_angle = 0;

		var zone_D_width = 30;
		var zone_D_height = 180.0;
		var zone_D_center_x = 0;
	...
	function update_view()
	{
		...
		// draw sole
		ctx.fillStyle = "#212121";
		ctx.fillRect(-SOLE_WIDTH/2, -1.2*SOLE_HEIGHT, SOLE_WIDTH, SOLE_HEIGHT);
	}
	...
	function ws_onopen()
	{
		document.getElementById("ws_state").innerHTML = "POVEZANO";
		document.getElementById("ws_state").style.fontWeight = "900";
		document.getElementById("ws_state").style.color = "#6dc900";
		document.getElementById("wc_conn").innerHTML = "Odspoji se";
	...
	function ws_onclose()
	{
		document.getElementById("ws_state").innerHTML = "NEPOVEZANO";
		document.getElementById("ws_state").style.fontWeight = "900";
		document.getElementById("ws_state").style.color = "#c90034";
		document.getElementById("wc_conn").innerHTML = "Poveži se";
	...
	function wc_onclick()
	{
		if(ws == null)
		{
			ws = new WebSocket("ws://<?echo _SERVER("HTTP_HOST")?>/botarm", "text.phpoc");
			document.getElementById("ws_state").innerHTML = "POVEZIVANJE";
			document.getElementById("ws_state").style.fontWeight = "300";
			document.getElementById("ws_state").style.color = "#ececec";
...
<body>
	...
	<div class="names">
		<h3 class="rame">RAME</h3>
		<h3 class="saka">ŠAKA</h3>
		<h3 class="zglob">LAKAT</h3>
	</div>

	<p><span id="ws_state">Nije spojeno</span><br></p>
	<button id="wc_conn" class="toggle" type="button" onclick="wc_onclick();">Poveži se</button>
</body>
</html>